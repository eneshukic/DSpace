...
importClass(Packages.org.dspace.app.xmlui.aspect.administrative.FlowRateRegistryUtils);
 ...
 */
function startManageRateRegistry()
{
	assertAdministrator();

	doManageRateRegistry();
...
/**
 * Manage the set of  rates that are available in the system. A
 * list of all available rates is given each one can be editeded or
 * deleteed and new ones may be added.
 */
function doManageRateRegistry()
{
    assertAdministrator();
    ...
    var result = null;
    do {
        sendPageAndWait("admin/rate-registry/main",{"updateID":updateID,"highlightID":highlightID},result);
        assertAdministrator();
		result = null;
        ...
        else if (cocoon.request.get("submit_edit") && cocoon.request.get("Id"))
        {
            var Id = cocoon.request.get("Id");
            result = doEditRate(Id);

            // Highlight the rate that was just edited.
            if (result != null && result.getParameter("Id"))
                highlightID = result.getParameter("Id");
            else
                highlightID = Id;
        }
        else if (cocoon.request.get("submit_add"))
        {
            ...
            result = doEditRate(-1);
             ...
             // Highlight the rate that was just created.
            if (result != null && result.getParameter("Id"))
                highlightID = result.getParameter("Id");
        }
        else if (cocoon.request.get("submit_delete") && cocoon.request.get("select_rate"))
        {
            // Delete the selected rates
            var IDs = cocoon.request.getParameterValues("select_rate");
            result = doDeleteRates(IDs);
            updateID = -1;
            highlightID = -1
        }
...
/**
 * Edit a single  rate specified by ID. If the ID is
 * -1 then a new rate will be added instead.
 */
function doEditRate(Id)
{
    assertAdministrator();

    var result = null;
    do {
        sendPageAndWait("admin/rate-registry/edit",{"Id":Id},result);
		assertAdministrator();
        result = null;
        ...
        {
            // Create or update an existing rate
            result = FlowRateRegistryUtils.processEditRate(getDSContext(), Id, cocoon.request);
        }
...
/**
 * Confirm the deletion of the listed bitstream rates
 */
function doDeleteRates(IDs)
{
    assertAdministrator();

    sendPageAndWait("admin/rate-registry/delete",{"IDs":IDs.join(',')});
	assertAdministrator();

    if (cocoon.request.get("submit_confirm"))
    {
        // Actually delete the rates
        var result = FlowRateRegistryUtils.processDeleteRates(getDSContext(),IDs)
        return result;
    }
    return null;
}
